...
#  
# 
#  Related Topics Array Math Stack 👍 5444 👎 847
...
class Solution:
    def evalRPN(self, tokens: List[str]) -> int:
        ...
        self.stack = []
        for token in tokens:
            if not self.stack and self.isOperator(token):
                return None
    ...
    def operate(self, token):
        ...
        match token:
            ...
            case '*':
                self.stack.append(first*second)
            case '/':
                # python round down for negative ints, needs math.ceil()
                if first*second >= 0:
                    self.stack.append(first//second)
                else:
                    self.stack.append(math.ceil(first/second))
